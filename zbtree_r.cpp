    ...
    for(int i=0; i<leaf_meta->max_use; i++)
    {
        if(leaf_meta->finger_print[i].sub_slot.hash_byte == fingerprint)
        {
...
#else
            if(leaf_meta->finger_print[i].sub_slot.control_bit >= leaf_meta->max_use) //cache hit, read directly from dram
            {
            ...
            else
            {
                ...
                if(kv_ptr[0] == key)
                {
                    if(leaf_meta->compact_use < MAX_LEAF_KEY - 1) //still have lodging space
                    {
                        bool lock_state = __sync_bool_compare_and_swap(&leaf_meta->temp_insert, 0, 1);
                        if(!lock_state)//check than insert 
                            return kv_ptr[1];
                        leaf_meta->finger_print[i].sub_slot.control_bit = leaf_meta->compact_use;
                        leaf_meta->finger_print[leaf_meta->compact_use].val = kv_ptr[0];
                        leaf_meta->compact_use++;
                        leaf_meta->finger_print[leaf_meta->compact_use].val = kv_ptr[1];
                        leaf_meta->compact_use++;
                        if(leaf_meta->finger_print[i].sub_slot.access_bit < 3)
                          leaf_meta->finger_print[i].sub_slot.access_bit++;
                    ...
                    {
                        if(leaf_meta->compact_full || leaf_meta->max_use >= MAX_LEAF_KEY - 2)
                        {
                            // lock_find.release();
                            return kv_ptr[1];
                        }
                        ...
                        bool is_full = false;
                        for(int j=0; j<leaf_meta->max_use; j++)
                        {
                            if(leaf_meta->finger_print[j].sub_slot.access_bit < leaf_meta->finger_print[i].sub_slot.access_bit &&  //access more times
                                  leaf_meta->finger_print[j].sub_slot.control_bit > leaf_meta->max_use) // is a valid place
                            {
                                // while(!__sync_bool_compare_and_swap(&leaf_meta->temp_insert, 0, 1));
                                bool lock_state = __sync_bool_compare_and_swap(&leaf_meta->temp_insert, 0, 1);
                                if(!lock_state)//check than insert 
                                    return kv_ptr[1];
                                leaf_meta->finger_print[i].sub_slot.control_bit = leaf_meta->finger_print[j].sub_slot.control_bit;
                                leaf_meta->finger_print[j].sub_slot.control_bit = 0;
                                leaf_meta->finger_print[leaf_meta->finger_print[i].sub_slot.control_bit].val = kv_ptr[0];
                                leaf_meta->finger_print[leaf_meta->finger_print[i].sub_slot.control_bit + 1].val = kv_ptr[1];
                                if(leaf_meta->finger_print[i].sub_slot.access_bit < 3)
                                  leaf_meta->finger_print[i].sub_slot.access_bit++;
                                is_full = true;
                                break;
...
restart:
    ...
    if(leaf_meta->max_use < MAX_LEAF_KEY)
    {
        ...
        if(insert_pos == leaf_meta->compact_use)
          leaf_meta->compact_use++;
        leaf_meta->temp_insert = 0;
        //write log
