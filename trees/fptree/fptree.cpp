...
#include "fptree.h"
uint64_t timer_search=0;
uint64_t timer_alc=0;
uint64_t count_alc=0;
uint64_t count_dlc=0;
uint64_t timer_write=0;
uint64_t timer_split=0;
...
FPtree::FPtree() 
{
    ...
    #endif
#ifdef LATENCY_TEST
    timer_split = end_count();
    std::cout<<"recover consumed "<<timer_split/1000<<" us "<<std::endl;
#endif

#ifdef EADR
    std::cout<<"using eadr mode, flush & clwb is disabled! "<<std::endl;
#endif
}
...
FPtree::~FPtree() 
{
    ...
    #ifdef LATENCY_TEST
    /*#elif LATENCY_TEST2
    std::cout<<"new node " << timer_counter<<"us"<<std::endl;
    std::cout<<"search " << timer_split <<"us"<<std::endl;
    std::cout<<"split "<<timer_flush<<"us"<<std::endl;*/
    #endif
    std::cout<<"alc consumed " << timer_alc<<std::endl;
    std::cout<<"persist "<<timer_write<<std::endl;
    std::cout<<"split total"<<timer_split<<std::endl;
    std::cout<<"search consumed " <<timer_search<<std::endl;  
    std::cout<<"alc is "<<count_alc<<std::endl;
    std::cout<<"dlc is "<<count_dlc <<std::endl;
}
                                            ...
                                            bool updateFunc = false, uint64_t prevPos = MAX_LEAF_SIZE)
{
    ...
    if (decision == Result::Split)
    {
        ...
        // }
        //auto splitend = rdtsc();
        //timer_split += splitend - splitstart;
    }
    ...
    {   
        ...
        {
            //count_dlc += sizeof(struct InnerNode);
            cur = new InnerNode();
...
void FPtree::updateParents(uint64_t splitKey, InnerNode* parent, BaseNode* child) 
{
    ...
    while (true)
    {
        ...
        else 
        {
            ...
            if (parent == root)
            {
                //count_alc += sizeof(struct InnerNode);
...
uint64_t FPtree::rangeScan(uint64_t key, uint64_t scan_size, char* result)
{
    ...
    size_t records[scan_size + MAX_LEAF_SIZE];
    ...
    while (true) 
    {
        ...
        for (i = 0; i < MAX_LEAF_SIZE; i++)
        {
             ...
             {
                records[count] = leaf->kv_pairs[i].value;
                count++;
             }
        }
    ...
    i = count > scan_size? scan_size : count;
