...
void N::rebuild_node(N *node, std::vector<std::pair<uint64_t, size_t>> &rs,
    ...
    if(N::isLeafArray(node)){
        LeafArray* la = N::getLeafArray(node);
        if((uint64_t)la < start_addr || (uint64_t)la >= end_addr)
            return;
        size_t size = size_align(sizeof(LeafArray), 64);
        rs.push_back(std::make_pair((uint64_t)la, size));
        auto bit = la->bitmap.load();
        for(int i=0; i<LeafArrayLength; i++)
        {
            if(bit[i] == true)
            {
                Leaf* le= la->getLeafAt(i);
                size_t leaf_size = size_align(sizeof(Leaf) + le->key_len + le->val_len, 64);
                rs.push_back(std::make_pair((uint64_t)le, leaf_size));
            }
