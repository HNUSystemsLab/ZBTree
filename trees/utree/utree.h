...
size_t breakdown_write=0;
size_t breakdown_alloc=0;
using namespace std;
size_t count_dram = 0;
size_t count_pmem = 0;
inline void mfence()
...
void *alloc(size_t size) {
  auto begin_alc = read_rdtsc();
#ifdef USE_PMDK
  TOID(list_node_t) p;
  POBJ_ZALLOC(pop, &p, list_node_t, size);
  auto end_alc = read_rdtsc();
  breakdown_alloc += end_alc - begin_alc;
  //count_pmem += size;
...
class btree{
  ...
  public:
    ...
    int scan(entry_key_t, int, size_t*);
...
void openPmemobjPool() {
  ...
  if (file_exists(pathname) != 0) {
    ...
    if ((pop = pmemobj_create(pathname, POBJ_LAYOUT_NAME(btree),
                              (uint64_t)64 * 1024 * 1024 * 1024, 0666)) ==
        NULL) {
...
btree::~btree() { 
  ...
  pmemobj_close(pop); 
#endif
  std::cout<<"pmem alloc "<<count_pmem<<std::endl;
  std::cout<<"dram alloc "<<count_dram<<std::endl;
}
...
int btree::scan(entry_key_t key, int scan_size, size_t* result) {
  bool f = false;
  char *prev;
  char *ptr = btree_search_pred(key, &f, &prev);
  int scanned = 0;
