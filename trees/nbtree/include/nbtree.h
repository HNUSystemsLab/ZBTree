...
#define NBSIZE 1024*1024*1024*8UL
...
const uint64_t SPACE_PER_THREAD = 1024ULL * 1024ULL * 1024ULL;
...
__thread char *curr_pmem=nullptr;
__thread char *curr_dram=nullptr;

typedef tbb::speculative_spin_rw_mutex speculative_lock_t;
typedef speculative_lock_t::scoped_lock htm_lock;
using namespace std;

void* heap_start; //in NO PMDK mode
void* temp_start;
PMEMobjpool* nbpop;
void *data_alloc(size_t size)
{
  /*if(curr_pmem == nullptr)
  {
    space_lock.lock();
    curr_pmem = (char*)temp_start;
    temp_start =reinterpret_cast<void*>((size_t)temp_start + MEM_PER_THREAD);
    space_lock.unlock();
  }
  void *ret = curr_pmem;
  curr_pmem += size;
  return ret;*/
  PMEMoid paddr;
  pmemobj_alloc(nbpop, &paddr, size, 0, nullptr, nullptr);
  void* return_addr = pmemobj_direct(paddr);
...
{
 /*if(curr_dram == nullptr)//init first
  {
    void *mem = new char[SPACE_PER_THREAD];
    curr_dram = (char*) mem;
  }
  void *ret = curr_dram;*/
...
public:
  ...
  //PMEMobjpool* tree_pop;
  int c;
  btree();
  ~btree();
  void* palloc(size_t size);
  void* plalloc(size_t size);
  //void pfree(void* ptr);
...
private:
  ...
  void linear_find_leaf(entry_key_t key, leaf_node_t* start);
  friend class page;
...
{
  c++;
  anchor =new ((leaf_node_t*) leaf_alloc(sizeof(leaf_node_t))) leaf_node_t();
...
void* btree::palloc(size_t size){
  /*PMEMoid paddr;
  pmemobj_alloc(tree_pop, &paddr, size, 0, nullptr, nullptr);
  void* return_addr = pmemobj_direct(paddr);
#ifdef USE_PMDK
#endif
  return return_addr;*/
...
void* btree::plalloc(size_t size){
  /*PMEMoid paddr;
  pmemobj_alloc(tree_pop, &paddr, size, 0, nullptr, nullptr);
  void* return_addr = pmemobj_direct(paddr);
  //void *ret = curr_mem;
...
void btree::copy(leaf_node_t *leaf)
{
...
#ifndef eADR
  flush_data(firdata, sizeof(data_node_t) / 2);
  flush_data(secdata, sizeof(data_node_t) / 2);
  asm_mfence();
#endif
  ...
  leaf_node_t* temp_log = leaf->log;
  data_node_t* temp_data = leaf->data;
  // 5. commit copy
...
void btree::sync(leaf_node_t *leaf)
{
  ...
  leaf->sync_flag = true;
#ifndef eADR
  flush_data(&leaf->log->data, sizeof(data_node_t));
  flush_data(&leaf->log->next->data, sizeof(data_node_t));
#endif
  asm_mfence();
...
void btree::linear_find_leaf(entry_key_t key, leaf_node_t* start_leaf){
  while(start_leaf->next != nullptr)
  {
    for(int i=0; i<start_leaf->get_number(); i++)
    {
      if(start_leaf->data->kv[i].key == key)
      {
        start_leaf->print_node();
        abort();
      }
    }
    start_leaf = start_leaf->next;
  }
}
